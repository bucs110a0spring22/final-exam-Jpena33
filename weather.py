import requests
import os
from datetime import datetime

class Weather():

  def __init__(self,temp_city,weather_desc,hmdt,wind_spd,date_time):
    self.temp_city = temp_city
    self.weather_desc = weather_desc
    self.hmdt = hmdt
    self.wind_spd = wind_spd
    self.date_time = date_time
    
  def __str__():
    user_api = os.environ['current_weather_data']
    location = input("Enter city name: ")

    complete_api_link =   "https://api.openweathermap.org/data/2.5/weather?  q=binghamton&appid=weather_data"+user_api

    api_link = requests.get(complete_api_link)
    api_data = api_link.json()

    if api_data['cod'] == '404':
     print("Invalid City: {}, Please check city name",
   format(location))
    else:
     temp_city = ((api_data['main']['temp']) - 273.15)
     weather_desc = api_data['weather'][0]['description']
     hmdt = api_data['main']['humidity']
     wind_spd = api_data['wind']['speed']
     date_time = datetime.now().strftime("%d %b %Y | %I:%M:%S %p")

    print("Weather stats for - {}  || {}".format(location.upper(), date_time))
    print("Current temperature: {:.2f} deg C".format(temp_city))
    print("Current weather description: ",weather_desc)
    print("Current humidity: ",hmdt, '%')
    print("Current wind speed: ", wind_spd, 'kmph')
